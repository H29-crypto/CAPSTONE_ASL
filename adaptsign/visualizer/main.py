...
class Processor():
    ...
    def freeze_parameter(self, model):
        length = len(model.conv2d.visual.transformer.resblocks)
        for name, param in model.conv2d.named_parameters():
            if 'ln_post' not in name and 'Adapter' not in name and name.split('.')[-1]!='proj':
            #if 'ln_post' not in name and 'Adapter' not in name and name.split('.')[-1]!='proj' and 'resblocks.'+str(length-1) not in name:
            #if 'ln_post' not in name and 'ln_1' not in name and 'ln_2' not in name and 'Adapter' not in name and name.split('.')[-1]!='proj' and 'resblocks.'+str(length-1) not in name:
